...
export function ChatView({
  ...
  const membershipQuery = useQuery({
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("conversation_members").select("folder, accepted_at").eq("conversation_id", conversationId).eq("user_id", meId).maybeSingle();
  ...
  const messagesQuery = useQuery({
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("messages").select("id, sender_id, body, created_at").eq("conversation_id", conversationId).order("created_at", {
        ascending: true
      }).limit(200);
      if (error) throw error;
  ...
  return <div className="flex h-[calc(100svh-220px)] flex-col">
      <div className="flex items-center gap-2 border-b border-border/60 p-3 sm:p-4">
        {onBack ? <Button variant="ghost" size="icon" onClick={onBack} className="h-10 w-10">
      ...
      {isRequest ? <div className="border-b border-border/60 p-3 sm:p-4">
      ...
      <div className="flex-1 overflow-auto p-3 sm:p-4 space-y-2">
        {messagesQuery.isLoading ? <div className="text-sm text-muted-foreground">Carregando mensagens…</div> : messagesQuery.data?.length ? messagesQuery.data.map(m => {
        const mine = m.sender_id === meId;
        return <div key={m.id} className={mine ? "flex justify-end" : "flex justify-start"}>
                <div className={"max-w-[78%] rounded-2xl px-3 py-2 text-sm border border-border/60 " + (mine ? "bg-muted/20" : "invictus-surface")}>
                  {m.body}
                </div>
              </div>;
      ...
      <div className="border-t border-border/60 p-3 sm:p-4">
        ...
        void send();
          ...
          <Input value={text} onChange={e => setText(e.target.value)} placeholder={isRequest ? "Aceite para responder" : "Mensagem"} disabled={sending || isRequest} />
