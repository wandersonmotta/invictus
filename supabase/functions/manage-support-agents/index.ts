...
serve(async (req) => {
  ...
  try {
    ...
    // Check caller has admin or suporte_gerente role
    ...
    if (action === "create") {
      ...
      // Assign suporte role always
      await supabaseService.from("user_roles").insert({
        user_id: newUserId,
        role: "suporte",
      });

      // If position is "gerente", also assign suporte_gerente role
      if (position === "gerente") {
        await supabaseService.from("user_roles").insert({
          user_id: newUserId,
          role: "suporte_gerente",
        });
      }
    ...
    if (action === "remove") {
      ...
      // Remove both suporte and suporte_gerente roles
      await supabaseService
        .from("user_roles")
        .delete()
        .eq("user_id", targetUserId)
        .in("role", ["suporte", "suporte_gerente"]);
    ...
    if (action === "list") {
      // List all users with suporte role
      const { data: suporteRoles } = await supabaseService
        .from("user_roles")
        .select("user_id")
        .eq("role", "suporte");
      ...
      // Check which ones also have suporte_gerente role
    ...
    console.error("manage-support-agents error:", e);
    return new Response(
