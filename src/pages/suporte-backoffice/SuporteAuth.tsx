...
export default function SuporteAuth() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: hasRole, error: roleError } = await rpcUntyped<boolean>("has_role", {
      ...
      if (roleError || !hasRole) {
        ...
        toast({ variant: "destructive", title: "Acesso negado", description: "Você não tem permissão para acessar o suporte." });
        setLoading(false);
        return;
      }
      ...
      const defaultPath = isLovableHost(window.location.hostname)
        ? "/suporte-backoffice/dashboard"
        : "/dashboard";
      navigate(typeof from === "string" && from ? from : defaultPath, { replace: true });
    } catch (err: any) {
      toast({ variant: "destructive", title: "Erro ao entrar", description: err.message || "Verifique suas credenciais." });
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <div className="invictus-auth-frame relative z-10 my-4 w-full max-w-md max-h-[calc(100vh-2rem)] overflow-y-auto rounded-xl border border-primary/40 bg-black/60 p-8 backdrop-blur-md">
        <div className="mb-8 flex flex-col items-center gap-3">
          ...
          <span className="text-sm uppercase tracking-[0.25em] text-primary">Acesso Suporte</span>
        </div>
        <form onSubmit={handleLogin} className="space-y-5">
          <div className="space-y-2">
            <Label htmlFor="email" className="text-muted-foreground">E-mail</Label>
            <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="seu@email.com" required className="border-border bg-background/50" />
          </div>
          <div className="space-y-2">
            <Label htmlFor="password" className="text-muted-foreground">Senha</Label>
            <Input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" required className="border-border bg-background/50" />
          </div>
          <div className="pb-2">
