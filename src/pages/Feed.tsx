  ...
  return (
    <main className="invictus-page">
      ...
      <section className="space-y-6 pb-8">
        {feedQuery.isLoading ? (
          <div className="mx-auto w-full max-w-[480px] text-sm text-muted-foreground">Carregando…</div>
        ) : feedQuery.isError ? (
          <div className="mx-auto w-full max-w-[480px] text-sm text-muted-foreground">Não foi possível carregar o feed.</div>
        ) : feedQuery.data?.length ? (
          feedQuery.data.map((p) => <FeedPostCard key={p.post_id} post={p} />)
        ) : (
          <div className="mx-auto w-full max-w-[480px] text-sm text-muted-foreground">Ainda não há posts aqui.</div>
        )}
      </section>
    </main>
  );
