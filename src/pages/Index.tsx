...
const Index = () => {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      const {
        data
      } = await supabase.from("profiles").select("access_status, city, state, postal_code, location_lat, location_lng").eq("user_id", userId).maybeSingle();
      if (!alive) return;
  ...
  const nearby = React.useMemo(() => {
    ...
    const list = (nearbyPinsLive ?? []).map(p => ({
      pin: p,
  ...
  return <main className="invictus-page">
      <header className="invictus-page-header">
        ...
        <p className="invictus-lead">Somente membros aprovados (localização aproximada por cidade).</p>
      </header>

      <section className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <MemberMap pins={pinsForMap} centerMe={mapCenter} mapbox={MAPBOX_PREMIUM.token && MAPBOX_PREMIUM.style ? MAPBOX_PREMIUM : null} showRadius={mode === "nearby" && canUseProximity} radiusCenter={mode === "nearby" ? deviceLocation.approx : null} radiusKm={mode === "nearby" ? radiusKm : null} onSelectPin={userId => {
        ...
        <aside className="space-y-4">
            ...
            <CardContent className="space-y-3 text-sm">
              {!canUseProximity ? <p className="text-muted-foreground">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => setMode("global")} className={mode === "global" ? "invictus-surface invictus-frame h-10 flex-1 rounded-md text-sm font-medium" : "h-10 flex-1 rounded-md border border-border/60 text-sm text-muted-foreground hover:bg-muted/20"}>
                      Mapa global
                    </button>
                    <button type="button" onClick={() => setMode("nearby")} className={mode === "nearby" ? "invictus-surface invictus-frame h-10 flex-1 rounded-md text-sm font-medium" : "h-10 flex-1 rounded-md border border-border/60 text-sm text-muted-foreground hover:bg-muted/20"}>
                      Perto de mim
                    </button>
                  </div>

                  {mode === "nearby" ? <div className="space-y-3">
                      <div className="text-xs text-muted-foreground">Sua localização é aproximada (privacidade).</div>

                      {deviceLocation.status === "denied" ? <div className="space-y-2">
                          <p className="text-muted-foreground">
                            Você negou o acesso ao GPS. Para usar “Perto de mim”, habilite a permissão no navegador.
                          </p>
                          <button type="button" className="text-sm font-medium text-primary underline underline-offset-4" onClick={() => deviceLocation.start()}>
                            Tentar novamente
                          </button>
                        </div> : deviceLocation.status === "unsupported" ? <p className="text-muted-foreground">Seu dispositivo não suporta geolocalização.</p> : <>
                          <div className="space-y-2">
                            <div className="flex items-center justify-between gap-3">
                              ...
                              <div className="text-xs text-muted-foreground">
                                {deviceLocation.status === "requesting" ? "Solicitando GPS…" : !gpsReady ? "Aguardando GPS…" : null}
                              </div>
                            </div>
                            <Slider value={[radiusKm]} min={10} max={200} step={10} onValueChange={v => setRadiusKm(v[0] ?? 80)} />
                          </div>
                          ...
                          {nearby && nearby.length ? <div className="max-h-72 overflow-auto rounded-lg border border-border/60">
                              <ul className="divide-y divide-border/60">
                                {nearby.slice(0, 25).map(({
                        pin,
                        distanceKm
                      }) => <li key={pin.user_id}>
                                    <button type="button" onClick={() => setSelectedUserId(pin.user_id)} className="flex w-full items-center gap-3 p-3 text-left hover:bg-muted/20">
                                      <img src={pin.avatar_url} alt={pin.display_name ? `Avatar de ${pin.display_name}` : "Avatar do membro"} className="h-10 w-10 rounded-full object-cover border border-border/70" loading="lazy" />
                                      <div className="min-w-0 flex-1">
                                        <div className="truncate text-sm font-medium">{pin.display_name ?? "Invictus"}</div>
                                        {pin.city && pin.state ? <div className="truncate text-xs text-muted-foreground">
                                            {pin.city}/{pin.state}
                                          </div> : null}
                                      </div>
                            ...
                            </div> : mode === "nearby" && gpsReady ? <p className="text-muted-foreground">Ninguém dentro do raio selecionado.</p> : null}
                        </>}
                    </div> : null}
                </>}
            </CardContent>
          ...
          <Card className="invictus-surface invictus-frame border-border/70">
            ...
            <CardContent className="space-y-2 text-sm">
              <p className="text-muted-foreground">
                {appearsOnMap ? "Você aparece no mapa (pin aproximado)." : "Você ainda não aparece no mapa (apenas aprovados + CEP salvo)."}
              </p>
              {me?.city && me?.state ? <p>
          ...
          <Card className="invictus-surface invictus-frame border-border/70">
            ...
            <CardContent className="space-y-2 text-sm">
              <p className="text-muted-foreground">
                {pinsLoading ? "Carregando pins..." : mode === "nearby" ? `${pinsForMap.length} pins (dentro do raio)` : `${pins.length} pins (membros aprovados)`}
              </p>
              <button type="button" className="text-sm font-medium text-primary underline underline-offset-4" onClick={() => void reload()}>
                Recarregar
              </button>

              {mode === "nearby" && canUseProximity ? <button type="button" className="text-sm font-medium text-primary underline underline-offset-4" onClick={() => void reloadNearby()}>
      ...
      <MemberQuickProfileDialog userId={selectedUserId} open={!!selectedUserId} onOpenChange={o => {
